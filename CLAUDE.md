...
`penumbra-glsl-01-cpig-gyroid` drives CPig live preview through the **in-process GLSL conduit** (Swath C): the Slop loader builds the gyroid field, `RunCommand _PenumbraPreviewBackend _Glsl` sets the backend, then Build → CPig's `.3mf` push routes to a **device-free node compile** (`compileThreeMFToGlArtifact`) and renders by ray-marching into Rhino's GL context. **Capture is `source: viewport`** (NOT file-source): `CaptureToBitmap` includes the conduit's `PostDrawObjects` GL draw (the Swath-4 lesson — only `DrawForeground`-blit overlays are uncapturable). Gated by `WaitForPenumbraFrame` (now wired for the GLSL conduit: `PenumbraBridge.FrameStateProvider` reports the GLSL conduit when it's enabled, else the OOP one). **Run ISOLATED** (`runMode: fresh`) — the GLSL frame-ready revision is process-global. Requires the Penumbra Rhino plug-in auto-loaded + the CPig `.gha` (Penumbra Preview). MVP renders STRUCTURAL (tape) fields; a dense field logs a `gl.compile3mf` error (use the deprecated `penumbra` OOP suite for those until dense-field packing lands). `mode: capture` (no baseline; inspect the candidate + the `gl.scene.loaded`/`gl.compile3mf` telemetry).
