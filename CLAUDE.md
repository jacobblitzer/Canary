...
**Gotcha — Canary boots Penumbra with the shipping `default` feature profile**, which has `eventDrivenRender` (C2) ON since 2026-05-08: the render loop idles unless something marks dirty. Any test that polls a per-frame counter (e.g. `__canaryRunComputeMarcherSmoke`) must first run `window.__canarySetDisplayState({features: {eventDrivenRender: 'off'}})` as a setup command, or it starves at `dispatchCount=0` regardless of timeout — see BUG-0013 (`docs/bugs/0013-compute-smoke-starved-by-event-driven-render.md`) and penumbra ask 0002 for the upstream fix that will remove this requirement.
