...
#### Penumbra native-GLSL spike (`penumbra-glspike` suite, NEW 2026-06-15)
`penumbra-glspike-00-multi` guards the **in-process native-GLSL ray-march spike** (a *different* plugin from the `penumbra` studio overlay — `Penumbra.GlSpike.rhp`, branch `desktop-penumbra-rhino-glsl-spike`). It `RunCommand`s `PenumbraGlMulti` (five atoms: four separately-compiled tapes — sphere/box/torus/gyroid — concatenated into one tape texture + the Benchy mesh-SDF grid, dispatched in one frame) and **file-source-captures** the conduit's converged SDF dump. **Why file-source, not viewport:** the GlSpike draws in `PostDrawObjects` (a conduit overlay), which `CaptureToBitmap` cannot see (LESSONS #2) — so the Penumbra conduit dumps the converged frame to `%LOCALAPPDATA%\\Penumbra\\GlSpike\\active.png` (`glGetTexImage` readback) and the test captures that file. Requirements/footguns: **`Penumbra.GlSpike.rhp` registered for auto-load** (drag-in once, distinct from `Penumbra.Rhino`); the spike artifacts (`scene.multi.json` + `benchy.grid.f32`) present under `packages/host-node/spike`; **delete `active.png` before the run** (shared dump file → a failed converge captures the stale frame); the conduit dumps once the progressive ramp reaches full quality, so the checkpoint waits (`atTimeMs`). v1 `mode: capture` (no baseline). Throwaway-grade — graduates when the spike becomes a real plugin consuming `@penumbra/native`.
