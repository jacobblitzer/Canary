# Canary — Agent Guide Detail

> Overflow detail relocated from `AGENTS.md` so the front door stays under the ~8K
> auto-load limit. `AGENTS.md` is the map; this is the territory. Nothing here is new —
> it is the depth that used to sit below the auto-load cut and was silently dropped.
...
- **Debug-overhaul (shipped 2026-05-24)** — see [`docs/plans/2026-05-24-canary-debug-overhaul.md`](docs/plans/2026-05-24-canary-debug-overhaul.md) for the design + [`docs/progress/2026-05-24-canary-debug-overhaul.md`](docs/progress/2026-05-24-canary-debug-overhaul.md) for the per-phase log. Headline surfaces operators interact with:
  ...
  - **Feedback inbox** at `docs/feedback/{inbox,triaged,resolved}/` — operator Annotate flow + agent session-start scan.
...
1. **Update `AGENTS.md` in every affected repo.** This is the #1 priority — AGENTS.md is what the next session reads first. Add or revise the section describing the capability/contract. If a new pattern was established (like file-source checkpoints), document it where future sessions will find it.
...
**What triggers this:** Any change that would leave another repo's AGENTS.md or PEERS.md stale. Adding a `TestCheckpoint` field → update Pigture/CPig AGENTS.md. Adding an agent action → update repos whose tests use it. Changing test conventions → update Slop AGENTS.md if it affects JSON authoring.
...
docs/
  ...
  feedback/       # Operator-authored feedback inbox (see § Feedback inbox)
